        ...
        for key, val in funds.items():
            ...
            profile = FundProfile(
                ...
                target=TargetConfig(
                    ...
                    number_format=target_cfg.get("number_format"),
                    select_range=target_cfg.get("select_range", False),
                ),
            )
...
def _format_block(ws, start_cell: str, rows: int, cols: int, number_format: Optional[str]) -> None:
    if not number_format:
        return
    start = ws[start_cell]
    for r in range(rows):
        for c in range(cols):
            ws.cell(row=start.row + r, column=start.column + c).number_format = number_format


def _selection_range(start_cell: str, rows: int, cols: int) -> str:
    col_letter, row_idx = coordinate_from_string(start_cell)
    start_col = column_index_from_string(col_letter)
    end_col = start_col + cols - 1
    end_row = row_idx + rows - 1
...
def copy_exposure(raw_path: Path, template_path: Path, output_path: Path, profile: FundProfile) -> None:
    ...
    if table and profile.target.number_format:
        _format_block(
            target_ws,
            start_cell=profile.target.start_cell,
            rows=len(table),
            cols=len(table[0]),
            number_format=profile.target.number_format,
        )

    if table and profile.target.select_range:
        selection = _selection_range(
            start_cell=profile.target.start_cell,
            rows=len(table),
            cols=len(table[0]),
        )
        target_ws.sheet_view.selection = [Selection(sqref=selection, activeCell=profile.target.start_cell)]
